        ...
        <div class="bg-white dark:bg-gray-900 rounded-3xl max-w-5xl w-full p-6 sm:p-8 max-h-[90vh] overflow-y-auto animate-slideIn">
            ...
            <div class="flex flex-col lg:flex-row gap-6" style="max-height: 70vh;">
                ...
                <div class="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div id="rsv-menu-grid" class="grid grid-cols-2 sm:grid-cols-3 gap-3 overflow-y-auto flex-1 pb-4">
                        @foreach ($menus as $menu)
                            <div class="rsv-menu-item bg-gray-50 dark:bg-gray-800 rounded-2xl p-3 border border-gray-200 dark:border-gray-700 cursor-pointer hover:shadow-lg hover:border-emerald-500 transition group"
                                data-id="{{ $menu->id }}"
                                data-name="{{ $menu->name }}"
                                ...
                                @else
                                    <div class="w-full h-20 bg-gray-100 dark:bg-gray-700 rounded-xl mb-2 flex items-center justify-center">
        ...
        <script>
            ...
            function viewReservation(id) {
                ...
                fetch(routePrefix + '/reservasi/' + id)
                    .then(r => r.json())
                    .then(rsv => {
                        const statusLabels = {pending:'⏳ Pending',confirmed:'✅ Dikonfirmasi',cancelled:'❌ Dibatalkan',completed:'🎉 Selesai'};
                        let html = `
                        ...
                        if (rsv.orders && rsv.orders.length > 0) {
                            ...
                            rsv.orders.forEach(order => {
                                ...
                                html += `<div class="flex justify-between font-bold text-sm border-t border-gray-200 dark:border-gray-700 pt-2 mt-2"><span>Total</span><span>Rp ${parseInt(order.total_amount).toLocaleString('id-ID')}</span></div></div>`;
                            });
                        } else {
            ...
            function openStatusModal(id, currentStatus) {
                ...
                const statusFlow = {'pending':'confirmed','confirmed':'completed'};
            ...
            function rsvAddToCart(menuId) {
                ...
                if (existing) {
                    ...
                    rsvCart.push({ id: menu.id, name: menu.name, price: parseFloat(menu.price), qty: 1 });
                }
                rsvRenderCart();
            }
